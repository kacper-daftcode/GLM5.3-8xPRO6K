...
class DeepseekV32IndexerMetadataBuilder(AttentionMetadataBuilder):
    ...
    def build(
        ...
        if num_prefills > 0:
            ...
            # TQ_D2 trace: one line per step per rank with chunk metadata stats (sync!), file survives a kill.
            if _TQ_TRACE:
                _tq_trace_prefill(
                    chunks, common_attn_metadata, num_decodes, num_prefills, seq_lens, query_start_loc_cpu
                )
...
# ---- TQ_D2 tracing (debug only; enabled with VLLM_TQ_INDEXER_TRACE=1) -----------------------------------
import os as _os
...
def _tq_trace_prefill(chunks, cam, num_decodes, num_prefills, seq_lens, qsl_cpu):
    global _tq_fh, _tq_step
    _tq_step += 1
    if _tq_fh is None:
        d = _os.environ.get("VLLM_TQ_INDEXER_TRACE_DIR", "/root/.cache/vllm/indexer_trace")
        _os.makedirs(d, exist_ok=True)
        _tq_fh = open(f"{d}/{_tq_rank_tag()}.log", "a", buffering=1)
        _tq_fh.write(f"# start {_time.strftime('%H:%M:%S')} pid={_os.getpid()}\n")
    torch.cuda.synchronize()
    try:
        sl_dev = seq_lens.tolist()
        sl_ub = cam.seq_lens_cpu_upper_bound.tolist() if cam.seq_lens_cpu_upper_bound is not None else None
        nct = cam.num_computed_tokens_cpu.tolist() if getattr(cam, "num_computed_tokens_cpu", None) is not None else None
        qsl = qsl_cpu.tolist()
        free_b, total_b = torch.cuda.mem_get_info()
        mem = (f"mem: used={(total_b - free_b) / 2**30:.2f}G torch_alloc={torch.cuda.memory_allocated() / 2**30:.2f}G "
               f"torch_res={torch.cuda.memory_reserved() / 2**30:.2f}G nontorch={(total_b - free_b - torch.cuda.memory_reserved()) / 2**30:.2f}G")
        line = (f"[{_time.strftime('%H:%M:%S')}] step={_tq_step} {mem} dec={num_decodes} pre={num_prefills} "
                f"seq_lens_dev={sl_dev[:6]} seq_lens_ub={None if sl_ub is None else sl_ub[:6]} "
                f"num_computed={None if nct is None else nct[:6]} qsl={qsl[:8]}")
        for i, c in enumerate(chunks):
            d = c.cu_seqlen_ke - c.cu_seqlen_ks
            bt = c.block_table
            ncols = min(bt.shape[1], (max(sl_ub) + 63) // 64) if sl_ub else bt.shape[1]
            bt_valid = bt[:, :ncols]
            line += (f" | c{i}: tok[{c.token_start},{c.token_end}) reqs={c.num_reqs} total={c.total_seq_lens} "
                     f"local={c.local_total_seq_lens} maxlocal={c.max_local_total_seq_lens} skip={c.skip_kv_gather} "
                     f"ks[{int(c.cu_seqlen_ks.min())},{int(c.cu_seqlen_ks.max())}] ke[{int(c.cu_seqlen_ke.min())},{int(c.cu_seqlen_ke.max())}] "
                     f"d[{int(d.min())},{int(d.max())}] bt{tuple(bt.shape)} btmax={int(bt.max())} bt0={int((bt_valid == 0).sum())}/{bt_valid.numel()} "
                     f"cusl={c.cu_seq_lens.tolist()[:4]}")
            if int(d.min()) <= 0 or int(c.cu_seqlen_ke.max()) > c.local_total_seq_lens:
                line += " <<<< BAD RANGE"
        # slot_mapping (where this step's K is written) vs block_table (where it is read back)
        try:
            sm = cam.slot_mapping
            bt_all = cam.block_table_tensor
            for r in range(num_decodes, num_decodes + num_prefills):
                q0, q1 = qsl[r], qsl[r + 1]
                nc = nct[r] if nct is not None else (sl_dev[r] - (q1 - q0))
                pos = torch.arange(nc, nc + (q1 - q0), device=sm.device)
                exp = bt_all[r, pos // 64].to(torch.int64) * 64 + pos % 64
                got = sm[q0:q1].to(torch.int64)
                mism = int((exp != got).sum())
                line += f" | slotchk r{r}: mism={mism}/{q1 - q0} sm[{int(got.min())},{int(got.max())}]"
                if mism:
                    line += " <<<< SLOT/BLOCK MISMATCH"
        except Exception as e:  # noqa: BLE001
            line += f" slotchk err {e!r}"
        _tq_fh.write(line + "\n")
    except Exception as e:  # noqa: BLE001
        _tq_fh.write(f"trace error: {e!r}\n")
