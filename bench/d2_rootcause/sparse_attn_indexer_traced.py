        ...
        for chunk in prefill_metadata.chunks:
            ...
            if chunk.local_total_seq_lens == 0:
                ...
                if current_platform.is_xpu():
                    ...
                    if _TQ_TRACE:
                        _tq_before(k_cache_prefix, chunk, k_quant, k_scale, q_slice, weights, use_fp4_cache)
                    ...
                    if _TQ_TRACE:
                        _tq_after_logits(k_cache_prefix, chunk, logits, k_quant, k_scale, cu_seqlen_ks, cu_seqlen_ke, chunk.block_table)
                ...
                if _TQ_TRACE:
                    _tq_after_topk(k_cache_prefix, chunk, topk_indices)
...
# ---- TQ_D2 tracing (debug only; VLLM_TQ_INDEXER_TRACE=1) --------------------------------------------------
import os as _os
import time as _time

_TQ_TRACE = _os.environ.get("VLLM_TQ_INDEXER_TRACE") == "1"
_tq_dir = _os.environ.get("VLLM_TQ_INDEXER_TRACE_DIR", "/root/.cache/vllm/indexer_trace")
_tq_state: dict = {"fh": None, "last": None, "n": 0, "kstats": ""}
...
def _tq_init():
    if _tq_state["fh"] is None:
        _os.makedirs(_tq_dir, exist_ok=True)
        tag = _tq_tag()
        _tq_state["fh"] = open(f"{_tq_dir}/{tag}.kernels.log", "a", buffering=1)
        _tq_state["last"] = open(f"{_tq_dir}/{tag}.last.txt", "w")
        _tq_state["tag"] = tag


def _tq_write_last(msg):
    f = _tq_state["last"]
    f.seek(0); f.truncate(); f.write(msg + "\n"); f.flush()


def _tq_before(layer, chunk, k_quant, k_scale, q_slice, weights, use_fp4):
    _tq_init()
    _tq_state["n"] += 1
    n = chunk.local_total_seq_lens
    if not chunk.skip_kv_gather:  # gather happened for this chunk: inspect K once per step/layer
        kq = k_quant[:n]
        ks = k_scale[:n]
        if use_fp4:
            kq_nan = -1; ks_f = ks.view(torch.int32).float()
        else:
            kq_nan = int(torch.isnan(kq.float()).sum()); ks_f = ks.view(torch.float32)
        _tq_state["kstats"] = (f"k_nan={kq_nan} kscale_nan={int(torch.isnan(ks_f).sum())} kscale_inf={int(torch.isinf(ks_f).sum())} "
                               f"kscale_absmax={float(ks_f.abs().max()):.3g} kscale_zero={int((ks_f == 0).sum())}")
    w = weights[chunk.token_start : chunk.token_end]
    q_nan = int(torch.isnan(q_slice.float()).sum()) if not use_fp4 else -1
    msg = (f"[{_time.strftime('%H:%M:%S')}] n={_tq_state['n']} L={layer} tok[{chunk.token_start},{chunk.token_end}) local={n} "
           f"skip={chunk.skip_kv_gather} {_tq_state['kstats']} q_nan={q_nan} w_nan={int(torch.isnan(w).sum())} w_absmax={float(w.abs().max()):.3g} "
           f"-> BEFORE mqa_logits")
    _tq_write_last(msg)


def _tq_after_logits(layer, chunk, logits, k_quant, k_scale, ks, ke, bt):
    nan = int(torch.isnan(logits).sum()); inf = int(torch.isinf(logits).sum())
    msg = f"logits{tuple(logits.shape)} nan={nan} inf={inf} absmax={float(logits.nan_to_num().abs().max()):.3g} -> BEFORE topk"
    _tq_write_last(_tq_state["last"].name and msg)
    if nan or inf:
        _tq_state["fh"].write(f"[{_time.strftime('%H:%M:%S')}] ANOMALY L={layer} tok[{chunk.token_start},{chunk.token_end}) {msg} {_tq_state['kstats']}\n")
        d = f"{_tq_dir}/dump_{_tq_state['tag']}_{_tq_state['n']}.pt"
        if not _os.path.exists(d):
            torch.save({"logits_nan_rows": torch.isnan(logits).any(dim=1).nonzero().cpu(), "ks": ks.cpu(), "ke": ke.cpu(),
                        "k_scale": k_scale[: chunk.local_total_seq_lens].cpu(), "block_table": bt.cpu(),
                        "layer": layer, "token_start": chunk.token_start, "token_end": chunk.token_end}, d)


def _tq_after_topk(layer, chunk, topk_indices):
    mn = int(topk_indices.min()); mx = int(topk_indices.max())
    _tq_state["fh"].write(f"[{_time.strftime('%H:%M:%S')}] n={_tq_state['n']} L={layer} tok[{chunk.token_start},{chunk.token_end}) "
                          f"local={chunk.local_total_seq_lens} {_tq_state['kstats']} topk[{mn},{mx}] OK\n")
    _tq_write_last("idle after topk n=%d" % _tq_state["n"])
