...
def _select_swap_blocks_fn(
    ...
    if gpu_to_cpu:
        # TQ_D2 (A/B, 2026-09-21): VLLM_TQ_OFFLOAD_D2H=triton routes the GPU->CPU stores through the Triton SM kernel
        # instead of the C++ cuMemcpyBatchAsync path, to test whether the batched copy-engine memcpy is what hangs the GPU.
        if os.environ.get("VLLM_TQ_OFFLOAD_D2H") == "triton" and HAS_TRITON:
            import vllm.v1.kv_offload.cpu.swap_blocks_triton as _sbt

            _sbt.MIN_N = 0  # never fall back to the C++ path
            ...
            logger.warning("TQ_D2: GPU->CPU offload copies via Triton SM kernel (chunk %d B)", chunk)
